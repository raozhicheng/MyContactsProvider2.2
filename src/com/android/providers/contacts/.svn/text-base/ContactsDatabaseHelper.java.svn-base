...
/* package */ class ContactsDatabaseHelper extends SQLiteOpenHelper {
    ...
    static final int DATABASE_VERSION = 310;
    ...
    public interface Views {
        ...
        public static final String CUSTOM_CONTACTS="view_custom_contacts";
...
//        ");");
    ...
    private static void createContactsViews(SQLiteDatabase db) {
        ...
        /*Add begin, custom by 139's contacts*/ 
		db.execSQL("DROP VIEW IF EXISTS " + Views.CUSTOM_CONTACTS + ";");

		String customSelect = "SELECT " + ContactsColumns.CONCRETE_ID + " AS "
				+ Contacts._ID + "," + RawContacts.DISPLAY_NAME_SOURCE + ", "
				...
				+ ", " + Data.DATA1 + ", " + Data.DATA2 + ", " + RawContacts.DELETED + ", "
				+ RawContacts.DISPLAY_NAME_ALTERNATIVE + " FROM "
				+ Tables.CONTACTS + " LEFT OUTER JOIN " + Tables.RAW_CONTACTS
				+ " ON ( " + RawContactsColumns.CONCRETE_CONTACT_ID + " = "
				+ ContactsColumns.CONCRETE_ID
				+ " )LEFT OUTER JOIN (SELECT * FROM " + Tables.DATA + " , "
				+ Tables.MIMETYPES + " WHERE "
				+ DataColumns.CONCRETE_MIMETYPE_ID + " = "
				+ MimetypesColumns.CONCRETE_ID + " AND "
				+ MimetypesColumns.CONCRETE_MIMETYPE + " = '"
				+ Phone.CONTENT_ITEM_TYPE + "' AND "
				+ DataColumns.CONCRETE_DATA2 + " = '" + Phone.TYPE_MOBILE
				+ "') AS A ON (A." + Data.RAW_CONTACT_ID + " = "
				+ RawContactsColumns.CONCRETE_ID + ")";

		db.execSQL("CREATE VIEW " + Views.CUSTOM_CONTACTS + " AS "
				+ customSelect);
...
//          if("sort_key".equals(toKey) || "sort_key_alt".equals(toKey))
//          {
//          	String ort_key = ContactLocaleUtils.getIntance().getSortKey(fromValues.getAsString(fromKey));
//          	toValues.put(toKey, ort_key);
//          }
//          else
//          {
...
//          }        	
    ...
    /*Add begin, custom by 139's contacts*/ 
    public String getCustomContactsView(){
    	return Views.CUSTOM_CONTACTS;
    }
    /*End begin, custom by 139's contacts*/ 
